...
class Player(pygame.sprite.Sprite):
    ...
    def changeSpeed(self, direction ):
        if direction[0] < 0:
            self.image = pygame.transform.flip(self.base_image, True, False) #to make the pacman mouth flip

        if direction[0] > 0:
            self.image = self.base_image.copy()

        if direction[1] < 0:
            self.image = pygame.transform.rotate(self.base_image, 90)   #for up image of pacman
